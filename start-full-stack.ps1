...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$basePath\ecommerce-backend\discovery-service'; $env:Path='$env:Path'; mvn spring-boot:run"
Write-Host "Waiting 20s for Eureka..."
Start-Sleep -Seconds 20

# 3. Start API Gateway
Write-Host "Starting API Gateway..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$basePath\ecommerce-backend\api-gateway'; $env:Path='$env:Path'; mvn spring-boot:run"
Start-Sleep -Seconds 10

# 4. Start Microservices
Write-Host "Starting Auth Service..." -ForegroundColor Blue
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$basePath\ecommerce-backend\auth-service'; $env:Path='$env:Path'; mvn spring-boot:run"

Write-Host "Starting Catalog Service..." -ForegroundColor Magenta
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$basePath\ecommerce-backend\catalog-service'; $env:Path='$env:Path'; mvn spring-boot:run"
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$basePath\ecommerce-backend\order-service'; $env:Path='$env:Path'; mvn spring-boot:run"
